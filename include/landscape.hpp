...
protected:
    ...
    const float height_of_tree_{10.0};
    const float tree_log_radius_{2.0};
...
public:
    std::shared_ptr<threepp::PlaneGeometry> groundGeometry = threepp::PlaneGeometry::create(
        ground_width_, ground_heigth_);
    std::shared_ptr<threepp::MeshStandardMaterial> groundMaterial = threepp::MeshStandardMaterial::create({
        {"color", threepp::Color::green}
    });
    ...
    virtual ~Landscape() = default; //For inheritance
